"""
Create the podcast_chunks index in Elasticsearch.

Phase 4 (Hybrid Retrieval), sub-phase 4.1.3. Creates the index from the
mapping at scripts/elasticsearch/mappings/podcast_chunks.json. Does NOT
ingest documents — bulk ingestion is 4.1.4.

Usage:
    python scripts/elasticsearch/create-index.py           # safe mode, skip if exists
    python scripts/elasticsearch/create-index.py --force    # delete and recreate if exists
...
MAPPING_PATH = Path(__file__).parent / "mappings" / "podcast_chunks.json"
...
def create_index(force: bool = False) -> None:
    ...
    # elasticsearch-py 8.x: pass settings/mappings as explicit kwargs (the old
    # `body=` form is deprecated and warns).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create the podcast_chunks Elasticsearch index.")
    parser.add_argument(
        "--force",
        action="store_true",
