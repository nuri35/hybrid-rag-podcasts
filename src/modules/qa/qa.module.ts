 ...
 *
 * `RetryPolicyService` is provided and exported here ahead of being
 * wired into anything — Phase 3 of the retry sprint will add a
 * `ResilientLlmService` that consumes it. Until then it has no
 * runtime callers; the provider registration is what proves the DI
 * container can resolve it end-to-end.
 */
@Module({
  imports: [RetrievalModule, LlmModule, RedisModule, VectorStoreModule],
  controllers: [QaController],
  providers: [QaChainService, RetryPolicyService],
  exports: [QaChainService, RetryPolicyService],
})
