...
10. **Two-layer rate limiting for the embedding API.** Gemini Tier 1 enforces approximately 15 RPM on the embedding endpoints. The default LangChain retry uses a 1s → 2s → 4s exponential backoff that is shorter than Gemini's 60 s rate-limit reset window, so retries fire while still rate-limited and never escape the state. Worse, `@langchain/google-genai` 0.2.x's `_embedDocumentsContent` *swallows* `batchEmbedContents` rejections by substituting `Array(N).fill([])`, so the 429 looks like a success at the wrapper boundary. Two layers fix this:

    - **Token bucket (proactive)** — `waitForToken()` reserves an atomic slot via a monotonically advancing `lastRequestTime`, hard-capping requests at `EMBEDDING_REQUESTS_PER_MINUTE` (default 15 = one request per 4 s). The reservation pattern (advance the timestamp BEFORE awaiting) keeps the gate correct under concurrent batches; two parallel callers each get unique slots spaced `minIntervalMs` apart.
    - **Adaptive retry (reactive)** — `embedWithAdaptiveRetry()` wraps every batch call. On a thrown 429 OR on the SDK silent-empty-array substitution (`v.length === 0`), it waits `EMBEDDING_RETRY_INITIAL_DELAY_MS` (default 200 ms) and re-attempts, growing the delay by `EMBEDDING_RETRY_GROWTH_FACTOR` (default 1.5) up to `EMBEDDING_RETRY_MAX_DELAY_MS` (default 2000 ms), for up to `EMBEDDING_RETRY_MAX_ATTEMPTS` (default 10). Non-429 errors fail fast — no retry wasted. Token bucket is reacquired on every attempt so the retry honours the rate.

    LangChain's internal retry is disabled (`maxRetries: 0`) so all retry logic lives in one place. Atomic-success is preserved: if any batch exhausts its retry budget, `Promise.allSettled` records the rejection and `EmbeddingFailedException` aborts the pipeline. Tier-aware via env — Tier 2 sets `EMBEDDING_REQUESTS_PER_MINUTE=60` (~15 min full ingest), Tier 3 sets `200+` (~10 min). Pattern matches Stripe / AWS / Pinecone client behaviour.
