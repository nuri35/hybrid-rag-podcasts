...
9. **Embedding provider is Google Gemini `text-embedding-004` (768 dim, free tier).** **⚠ Deprecation watch (2026-06-05, Phase 2 eval):** `text-embedding-004` was removed from the Generative Language API's ListModels and returns 404 on v1beta `embedContent` (Python SDK path), yet the JS SDK path used by production retrieval still serves successfully — deprecated-but-functional. If query embedding starts failing, the fix is a provider/model switch + full re-ingestion (vector spaces differ). The Phase 2 Ragas judge already moved to `gemini-embedding-001` (judge embeddings are independent of the Chroma space). Parallelism is 5 in-flight batches via `p-limit`; batch failures are isolated via `Promise.allSettled`; LangChain handles per-batch retry (3 attempts). Switching provider requires re-ingestion since vector spaces differ. (`@google/generative-ai` is installed at root with `--legacy-peer-deps` because chromadb 1.x declares a stale `^0.1.1` peer dep that doesn't affect actual chromadb usage — see `package.json`.)
